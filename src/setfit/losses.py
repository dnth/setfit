...
class SupConLoss(nn.Module):
    ...
    def compute_loss_from_embeddings(self, embeddings, labels=None, mask=None):
        """Compute loss from pre-computed embeddings.

        Args:
            embeddings: List of embedding tensors, each of shape [emb_dim]
        ...
        Returns:
            A loss scalar.
        """
        features = torch.stack(embeddings, dim=0)  # [bsz, emb_dim]

        # Normalize embeddings
        features = torch.nn.functional.normalize(features, p=2, dim=1)

        # Add n_views dimension
        features = torch.unsqueeze(features, 1)  # [bsz, 1, emb_dim]
